...
import { hmacSha512 } from "../../hmac/index.mjs"; // performs hard-only derivation on the xprv
...
export function ledgerDerivePrivate(xprv, index) {
  ...
  const left = bnToU8a(new BN(kl, 16, 'le').add(new BN(zl.slice(0, 28), 16, 'le').mul(new BN(8))), {
    bitLength: 512,
    isLe: true
  }).slice(0, 32);
  const right = bnToU8a(new BN(kr, 16, 'le').add(new BN(zr, 16, 'le')), {
    bitLength: 512,
    isLe: true
  }).slice(0, 32);
